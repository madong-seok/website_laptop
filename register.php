        ...
        <div class="mainres">
           ...
           <form method="POST" enctype="multipart/form-data">
            <div class="titleres">
                ...
                <hr></hr>            
                <p></p>
				<input class="textres" type="text" placeholder="Nhập tên người dùng" name="txt_usname"/> 
                <input class="textres" type="text" placeholder="Nhập email của bạn" name="txt_email"/>  
				...
				<p style="margin-left:10px; margin-top: 10px;">Số điện thoại</p>
                <input class="textres" type="text" name="txt_number"  />
                ...
                <input class="textres" type="text" name="address" placeholder="Nhập địa chỉ thanh toán">
                <p style="margin-left:10px; margin-top: 10px; font-size:7pt;">Bằng cách nhấp vào Đăng ký, bạn đồng ý với <a href="#">Điều khoản</a>, <a href="#">Chính sách dữ liệu</a> và <a href="#">Chính sách cooki</a>  của chúng tôi. Bạn có thể nhận được thông báo của chúng tôi qua Email và hủy nhận bất kỳ lúc nào.</p>
           ...
           <?php 
            ...
            if(isset($_POST['sub_dk'])){
                ...
                if(empty($_POST['txt_usname'])||empty($_POST['txt_passwd'])||empty($_POST['txt_email']))
                echo "Chưa nhập đủ dữ liệu!";
				
                else{
					$email = test_input($_POST["txt_email"]);
					if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
						echo "<script>alert('Vui lòng nhập email đúng định dạng !')</script>";
					  }
                elseif($_POST['txt_passwd']!=$_POST['re_passwd'])
                echo "Mật khẩu nhập lại không đúng!";
				
                else{
                        
                    $ins= $get_data -> ins_user($_POST['txt_usname'],$_POST['txt_email'],$_POST['txt_passwd'],$_POST['txt_number'],$_POST['gender'],$_POST['birthday'],$_POST['address'])	;
